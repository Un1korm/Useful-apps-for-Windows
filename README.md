...
| [Discord (Мессенджер)](#discord-геймерская-платформа) | [BleachBit (Очистка системы)](#bleachbit-очистка-системы) | [Caesium (Сжатие фото)](#caesium-image-compressor-сжатие-изображений) |
...
| | [WinClick (Оптимизатор)](#winclick-оптимизация-windows) | [Obsidian (Заметки)](#obsidian-программа-для-заметок) |
...
| [WinDynamicDesktop (Динамические обои)](#windynamicdesktop-динамические-обои) | [PyCharm (Python IDE)](#pycharm-редактор-кода-python) | [iMazing (Резервная копия iOS)](#imazing-резервная-копия-ios) |
...
| | | [Устройства Apple (Сброс и обновление)](#устройства-apple-сброс-и-обновление) |
...
Интегрированная среда разработки (IDE) для языка Python от компании JetBrains. PyCharm предоставляет мощные инструменты для написания кода: умное дополнение кода, анализ ошибок в реальном времени, мощный отладчик и встроенную поддержку систем контроля версий (Git, Mercurial и др.). Доступна в двух версиях: бесплатной Community (с открытым исходным кодом) и платной Professional с расширенными возможностями для веб-разработки. Приложение доступно на платформах Windows, macOS и Linux.
...
Скачать приложение:
  ...
  - [Microsoft Store](https://apps.microsoft.com/detail/xp9khm4bk9fz7q?hl=ru-RU&gl=RU)

Мощный, легкий и бесплатный редактор исходного кода от Microsoft. Благодаря огромной экосистеме расширений, VS Code поддерживает практически все языки программирования (Python, C++, Java, JavaScript, Go и др.), фреймворки и инструменты. Имеет встроенную поддержку Git, терминал и умный автодополнитель IntelliSense. Приложение доступно на платформах Windows, macOS и Linux.


## iMazing (Резервная копия iOS):
![](logo_img/imazing-img.webp)

Скачать приложение:
  - [Официальный сайт](https://imazing.com/ru/download)

Мощный менеджер для устройств Apple (iPhone, iPad, iPod), который предоставляет гораздо больше возможностей, чем стандартный iTunes. iMazing позволяет создавать резервные копии, управлять музыкой, фото и приложениями, а также сохранять на компьютер приложения, которые были удалены из App Store. Это незаменимый инструмент для тех, кто хочет иметь полный контроль над данными своего iOS-устройства. Приложение доступно на платформах Windows и macOS.


## AOMEI Backupper (Резервная копия ПК):
![](logo_img/aomei-backupper-img.png)
...
## Устройства Apple (Сброс и обновление):
![](logo_img/apps-apple-img.jpg)

Скачать приложение:
  - [Microsoft Store](https://apps.microsoft.com/detail/9np83lwlpz9k?hl=ru-RU&gl=RU)

Официальное приложение от Apple для синхронизации данных между устройствами (iPhone, iPad, Mac) и компьютером на Windows. Позволяет получить доступ к фотографиям, контактам, календарям, заметкам, закладкам Safari и паролям прямо на ПК. Поддерживает общий доступ к файлам через проводник и автоматическую загрузку фото из облака. Приложение доступно на платформе Windows.


## Zapret Discord Youtube (Обход блокировок DS/YT):
![](logo_img/zapret-img.jpg)

Скачать приложение:
  - [GitHub](https://github.com/Flowseal/zapret-discord-youtube)

Набор скриптов и утилит для обхода государственных блокировок (Deep Packet Inspection) таких платформ, как Discord и YouTube, на Windows. Проект основан на `zapret` от bol-van и предоставляет готовые стратегии обхода (ALT, FAKE, SIMPLE и другие). Программа может быть установлена как служба для автозапуска или запускаться вручную. **Важно:** Антивирусы могут ложно срабатывать на WinDivert — добавьте папку с программой в исключения. Доступно только для Windows.


## TG WS Proxy (Обход блокировки TG):
![](logo_img/tg-ws-proxy-img.png)

Скачать приложение:
  - [GitHub](https://github.com/Flowseal/tg-ws-proxy)

Локальный MTProto-прокси сервер для ускорения работы Telegram Desktop, который перенаправляет трафик через WebSocket-соединения. Приложение поднимает прокси на `127.0.0.1:1443`, перехватывает подключения к IP-адресам Telegram и устанавливает WebSocket-соединение (TLS) к соответствующим DC через домены Telegram. Данные передаются в зашифрованном виде, для работы не нужны сторонние серверы. Работает в системном трее — достаточно один раз настроить Telegram и забыть. Доступно для Windows 7/10/11, macOS и Linux.


## WinClick (Оптимизация Windows):
![](logo_img/winclick-img.png)

Скачать приложение:
  - [GitHub](https://github.com/MartyFiles/WinClick)

Настройка:
  - ["Optimize Windows 11 in 15 minutes! WinClick 2.0"](https://youtu.be/JMTuY_Gi70U) (MartyFiles)

Бесплатная open-source утилита для быстрой настройки и оптимизации свежеустановленной Windows 11. WinClick объединяет десятки твиков в одном интерфейсе: очистка системы, удаление предустановленных UWP-приложений, удаление Microsoft Edge и Защитника Windows, отключение телеметрии, оптимизация параметров производительности, настройка Центра обновлений, визуальные твики и даже сжатие системы. Рекомендуется запускать на чистой установке Windows 11 Pro (25H2+) **без подключения к интернету** для беспроблемного удаления Edge, WebView2 и Защитника. Доступно только для Windows 11.
